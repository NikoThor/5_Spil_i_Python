...
def convert_shape_format(shape):
    positions = []
    format = shape.shape[shape.roation % len(shape.shape)]
...
def draw_window(surface, grid):
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            pygame.draw.rect(surface, grid[i][j], (top_left_x + j*block_size, top_left_y + i*30, block_size, block_size),0)
...
win = pygame.display.set.mode(s_width, s_height)
